...
function parseTranslations(format, rawTranslations, options) {
  switch (format) {
    case '.yml':
    case '.yaml':
      return options.rootKey ? yaml.load(rawTranslations)[options.rootKey] : yaml.load(rawTranslations);
    case '.json':
      return options.rootKey ? JSON.parse(rawTranslations)[options.rootKey] : JSON.parse(rawTranslations);
    default:
