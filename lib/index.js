...
function parseTranslations(format, rawTranslations, options) {
  switch (format) {
    case '.yml':
    case '.yaml':
      return options.localeRootKey ? yaml.load(rawTranslations)[options.locale] : yaml.load(rawTranslations);
    case '.json':
      return options.localeRootKey ? JSON.parse(rawTranslations)[options.locale] : JSON.parse(rawTranslations);
    default:
...
function loadResources(locale, options) {
  ...
  return fs.readFile(file, options.encoding)
           .then(data => parseTranslations(extension, data, options));
}
